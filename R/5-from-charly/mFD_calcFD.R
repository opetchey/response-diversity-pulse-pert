...
speciesData <- readRDS(here(paste0("data/merged/species_response_traits.RDS")))
communityData <- readRDS(here(paste0("data/merged/comm_all.RDS")))
...
TraitData <- speciesData %>%
  mutate(case_id = paste(case_id, pack, sep="::")) |> 
  group_by(case_id) %>%
...
sp_tr <- TraitData %>%
  ...
  select(population, species_delta_pi_AUC, species_RR_AUC)%>%
...
#   faxes           = c('species_delta_pi_AUC','species_RR_AUC'),
#   name_file       = NULL,
#   faxes_nm        = NULL,
#   range_faxes     = c(NA, NA),
#   color_bg        = "grey95",
...
#saveRDS(alpha_fd_indices, "data/merged/alpha_fd_indices.RDS")

#xxx <- readRDS("data/merged/alpha_fd_indices.RDS")
...
#   plot_asb_nm              = c("Comm-98-rep-1",'Comm-91-rep-1'),
...
Fdiv <- FD_ind_values%>%
  ...
  select(case_id, fdis, feve, fric, community_id, replicate_id,
         comm_RR_AUC, OEV,
         mean_species_RR_AUC, var_species_RR_AUC,
         var_igr_effect, mean_igr_effect, RD_diss_igr_effect, RD_div_igr_effect)
