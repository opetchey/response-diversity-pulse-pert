...
packs_to_read_in <- c("pack1", "pack2", "pack4", "pack5", "pack6")
for(i in 1:length(packs_to_read_in)) {
  ...
  comm_time_stab <- dynamics |>
    ...
    group_by(case_id, community_id, replicate_id, Time, Treatment,alpha_ij_sd,alpha_ij_mean) %>%
  ...
  comm_stab <- comm_time_stab |>
    ...
    group_by(case_id, community_id, replicate_id,alpha_ij_sd,alpha_ij_mean) |> 
    summarise(comm_tot_deltabm = MESS::auc(Time, comm_deltabm),
              comm_tot_deltabm_raw = my_auc_func_raw(Time, comm_deltabm),
              OEV = MESS::auc(Time, abs(comm_deltabm)),
              OEV_sqrt = sqrt(abs(comm_tot_deltabm)),
              OEV_raw = abs(comm_tot_deltabm_raw))
  ...
  species_time_stab <- dynamics |>
    ...
    ungroup()|>
    mutate(RR = (Perturbed - Control)/
             (Perturbed + Control), # biomass response ratio for biomass stability
           delta.pi = (Perturbed/treat.tot)-(Control/con.tot))  # proportional change for proportional stability
  
  species_stab <- species_time_stab |>
    filter((Time %% keep_every_t) == 0) |> 
    group_by(case_id, community_id, replicate_id, Species_ID,alpha_ij_sd,alpha_ij_mean) |> 
    #calculate AUC using MESS package with linear splines
    summarise(species_AUC.RR = MESS::auc(Time, RR),
              species_AUC.pi = MESS::auc(Time, delta.pi))
  ...
  comm_indicies <- species_stab |> 
    ...
    summarise(mean_spp_deltabm = mean(species_AUC.RR),
              var_spp_deltabm = var(species_AUC.RR),
              mean_spp_deltabm_pi = mean(species_AUC.pi),
              var_spp_deltabm_pi = var(species_AUC.pi))
