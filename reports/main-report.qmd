...
This was calculated as the effect of the pulse perturbation on the total community biomass. Specifically the area between the control and perturbed dynamics. Stability is greatest when this is zero. More negative and more positive values imply lower stability. To make a measure of stability, we take the absolute value of this. Both the actual value and the absolute are used below, in different settings.
...
```{r}
source(here("R/single_community_plots.R"))
p_resp_curves
p_dynamics
traits_table
p_commstab1
