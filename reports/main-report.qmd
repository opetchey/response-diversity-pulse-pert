...
expl_all_long <- expl_all %>%
  pivot_longer(names_to = "variable", values_to = "rsq", cols = 3:10) %>%
...
#| fig-height: 8
comm_all |> 
  ggplot(aes(x = mean_igr_effect, y = mean_species_RR_AUC)) +
  geom_point() +
...
```{r}
#| echo: false
expt_p7 <- readRDS(here("data/pack7/expt_communities.RDS")) |> 
...
comm_all_p7 <- readRDS(here("data/pack7/community_measures.RDS")) |> 
  full_join(expt_p7)
...
```{r}
comm_all_p7 |> 
  ggplot(aes(x = mean_igr_effect, y = comm_RR_AUC, col = b_opt_range)) +
  geom_point() +
...
```{r}
comm_all_p7 |> 
  ggplot(aes(x = b_opt_range, y = comm_RR_AUC, col = mean_igr_effect)) +
  geom_point() +
  facet_wrap(vars(alpha_ij_sd))
```
