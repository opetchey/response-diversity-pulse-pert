...
```{r}
#saveRDS(expt, here("experiments/expt_communities.RDS"))
```
...
```{r}
#saveRDS(temperature_treatments, here("experiments/temperature_treatments.RDS"))
```
...
```{r}
#| echo: false
data_specification <- read_excel(here("experiments/data_specification.xlsx"),
                                 sheet = "metadata")
data_specification %>% 
  kbl() %>%
  kable_styling()
